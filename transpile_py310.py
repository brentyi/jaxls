"""Transpile Python 3.12+ source files to Python 3.10 compatibility.

Primary function is to remove Python typing features that are only supported in
newer versions of Python (see PEP 585, PEP 695).

This is not meant to be a comprehensive transpiler; it is only intended
for use in jaxls.
"""
...
class PythonTranspiler(cst.CSTTransformer):
    ...
    def leave_AnnAssign(self, original_node, updated_node):
        ...
        if updated_node.annotation:
            ...
            # Remove type aliases (PEP 613 style)
            if isinstance(annotation, cst.Name) and annotation.value == "TypeAlias":
                return cst.RemovalSentinel.REMOVE

            if isinstance(annotation, cst.Subscript) and self._is_special_wrapper(
                annotation
            ):
    ...
    def _is_special_wrapper(self, subscript):
        ...
        if isinstance(subscript.value, cst.Name):
            ...
            return isinstance(attr.attr, cst.Name) and attr.attr.value in [
                "Static",
                "ClassVar",
            ]
        return False
    ...
    def leave_ImportFrom(self, original_node, updated_node):
        ...
        if hasattr(original_node, "names") and isinstance(
    ...
    def _is_typing_import(self, node):
        ...
        return node.module and node.module.value in (
            "typing",
            "typing_extensions",
            "collections.abc",
        )
    ...
    def leave_SimpleStatementLine(self, original_node, updated_node):
        ...
        if (
            ...
            and updated_node.trailing_whitespace.comment
        ):
    ...
    def _has_class_getitem(self, class_node):
        ...
        for item in class_node.body.body:
            if (
                ...
                and item.name.value == "__class_getitem__"
            ):
                return True
        return False
    ...
    def _create_class_getitem_method(self):
        ...
        return cst.FunctionDef(
            ...
            params=cst.Parameters(
                params=[cst.Param(cst.Name("cls")), cst.Param(cst.Name("params"))]
            ),
            body=cst.IndentedBlock(
                body=[cst.SimpleStatementLine(body=[cst.Return(cst.Name("cls"))])]
            ),
    ...
    def leave_TypeAlias(self, original_node, updated_node):
        """Remove type aliases (PEP 695 style)."""
        return cst.RemovalSentinel.REMOVE

    def leave_Assign(self, original_node, updated_node):
        """Handle regular variable assignments that might be type aliases."""
        # Check if this looks like a type alias (assignment to a type)
        if (
            len(updated_node.targets) == 1
            and isinstance(updated_node.targets[0].target, cst.Name)
            and self._is_type_expression(updated_node.value)
        ):
    ...
    def _is_type_expression(self, node):
        """Check if expression is a type expression."""
        if isinstance(node, cst.Name):
            # Common type names that might indicate this is a type alias
            type_names = {
                "Type",
                "List",
                "Dict",
            ...
            return node.value in type_names
        elif isinstance(node, cst.Subscript):
            # Something like Union[str, int]
    ...
    def leave_Lambda(self, original_node, updated_node):
        ...
        new_params = updated_node.params.with_changes(
            params=[
                p.with_changes(annotation=None) for p in updated_node.params.params
            ],
        )
    ...
    def leave_Subscript(self, original_node, updated_node):
        ...
        if (
            ...
            and updated_node.value.value == "ClassVar"
        ):
            self.needs_any_import = True
            return updated_node.with_changes(
                slice=[cst.SubscriptElement(slice=cst.Index(value=cst.Name("Any")))]
    ...
    def leave_If(self, original_node, updated_node):
        ...
        if (
            isinstance(updated_node.test, cst.Name)
            and updated_node.test.value == "TYPE_CHECKING"
        ):
            return cst.RemovalSentinel.REMOVE
        ...
        # Handle if not TYPE_CHECKING
            ...
            if (
                isinstance(updated_node.test.operator, cst.Not)
                and isinstance(updated_node.test.expression, cst.Name)
                and updated_node.test.expression.value == "TYPE_CHECKING"
            ):
    ...
    def leave_Call(self, original_node, updated_node):
        """Handle cast() calls and PEP 585 constructors."""
        # Handle PEP 585 constructors like list[T](), dict[K, V]()
        if isinstance(updated_node.func, cst.Subscript):
            if isinstance(updated_node.func.value, cst.Name):
                # Check for built-in generic types
                generic_builtins = {"list", "dict", "set", "tuple"}
                if updated_node.func.value.value in generic_builtins:
                    # Replace with non-generic constructor
                    return updated_node.with_changes(func=updated_node.func.value)
...
def main() -> None:
    ...
    try:
        ...
        print(f"ruff check --fix failed: {e.stderr}")
